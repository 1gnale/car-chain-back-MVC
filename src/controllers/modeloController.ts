...
export class ModeloController {
    static async  getAllModelo(req: Request, res: Response) {
        try {
            ...
            return BaseService.success(res, finalModel, 'Modelos obtenidos exitosamente', finalModel.length);
        } catch (error: any) {
    ...
    try {
      ...
      const modeloModificado = await modelo.update({ descripcion, nombre, activo });
      if (activo) {
        await Version.update({activo: true}, {where: {modelo_id: id}})
      }
      return BaseService.success(res, modeloModificado, 'Modelo actualizado exitosamente');
    } catch (error: any) {
